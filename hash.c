...
static void F512(uint64_t *h, const uint64_t *m) {
...
static void F1024(uint64_t *h, const uint64_t *m) {
  static uint64_t y[COLS1024] __attribute__((aligned(16)));
  static uint64_t z[COLS1024] __attribute__((aligned(16)));
  static uint64_t outQ[COLS1024] __attribute__((aligned(16)));
  static uint64_t inP[COLS1024] __attribute__((aligned(16)));
  int i;
  ...
  for (i = 1; i < ROUNDS1024-1; i += 2) {
    RND1024Q(y,z,U64BIG((uint64_t)i));
    RND1024Q(z,y,U64BIG((uint64_t)(i+1)));
  }
  RND1024Q(y,outQ,U64BIG((uint64_t)(ROUNDS1024-1)));
	
  /* compute P(h+m) */
  RND1024P(inP,z,0);
  for (i = 1; i < ROUNDS1024-1; i += 2) {
    RND1024P(z,y,U64BIG(((uint64_t)i)<<56));
    RND1024P(y,z,U64BIG(((uint64_t)(i+1))<<56));
  }
  RND1024P(z,y,U64BIG(((uint64_t)(ROUNDS1024-1))<<56));
...
void Transform(hashState *ctx, 
  ...
  if (ctx->size == SHORT) {
    ...
    while (msglen >= SIZE512) {
      F512(ctx->chaining,(uint64_t*)input);
      msglen -= SIZE512;
      input += SIZE512;
    }
  }
    ...
    while (msglen >= SIZE1024) {
      F1024(ctx->chaining,(uint64_t*)input);
      msglen -= SIZE1024;
      input += SIZE1024;
    }
  }
...
void OutputTransformation(hashState *ctx) {
  ...
  case LONG  :
    ...
    for (j = 1; j < ROUNDS1024-1; j += 2) {
      RND1024P(y,z,U64BIG(((uint64_t)j)<<56));
      RND1024P(z,y,U64BIG(((uint64_t)j+1)<<56));
    }
    RND1024P(y,temp,U64BIG(((uint64_t)(ROUNDS1024-1))<<56));
    for (j = 0; j < COLS1024; j++) {
      ctx->chaining[j] ^= temp[j];
    }
    break;
...
HashReturn Init(hashState* ctx,
  ...
  if (hashbitlen <= 256) {
    ctx->size = SHORT;
    ctx->chaining = calloc(COLS512,sizeof(uint64_t));
    ctx->buffer = malloc(SIZE512);
    /* set initial value */
    ctx->chaining[COLS512-1] = U64BIG((uint64_t)hashbitlen);
  }
  else {
    ctx->size = LONG;
    ctx->chaining = calloc(COLS1024,sizeof(uint64_t));
    ctx->buffer = malloc(SIZE1024);
    /* set initial value */
    ctx->chaining[COLS1024-1] = U64BIG((uint64_t)hashbitlen);
  }
...
HashReturn Final(hashState* ctx,
  ...
  if (ctx->size == SHORT) {
    memset(ctx->chaining, 0, COLS512*sizeof(uint64_t));
    memset(ctx->buffer, 0, SIZE512);
  }
  else {
    memset(ctx->chaining, 0, COLS1024*sizeof(uint64_t));
